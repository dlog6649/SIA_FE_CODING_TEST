...
describe('LabelList 테스트 시작', () => {
  let wrapper = null;
  const _labels = [
    {id: 0, name: 'sea', coordinates: [{x: 10, y: 20}, {x: 110, y: 20}, {x: 110, y: 70}, {x: 10, y: 70}], data: {x: 10, y: 20, w: 90, h: 50, deg: 0}}
    ,{id: 1, name: 'woods', coordinates: [{x: 10, y: 20}, {x: 110, y: 20}, {x: 110, y: 70}, {x: 10, y: 70}], data: {x: 10, y: 20, w: 90, h: 50, deg: 0}}
    ,{id: 2, name: 'building', coordinates: [{x: 10, y: 20}, {x: 110, y: 20}, {x: 110, y: 70}, {x: 10, y: 70}], data: {x: 10, y: 20, w: 90, h: 50, deg: 0}}
  ];
  ...
  it('목록에 있는 라벨정보를 선택시 해당 라벨들에 대한 ID리스트가 디스패치돼야 한다.', () => {
    ...
    let selectLabelsEventByClick = mockSelectLabels.mock.calls;
    let [selectedLabelsIds] = selectLabelsEventByClick[0];
