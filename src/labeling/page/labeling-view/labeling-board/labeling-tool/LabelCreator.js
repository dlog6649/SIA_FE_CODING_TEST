...
export const redrawImage = (url, image) => {
  ...
  const img = document.createElementNS(labelNS.svgNS, "image")
...
export const redrawLabels = (labels) => {
  const _labels = [...labelNS.svg.childNodes].filter((node) => node.classList.contains("label"))
  ...
  labels.forEach((label) => {
    const { x } = label.data
    const { y } = label.data
    const width = label.data.w
    const height = label.data.h
    const { deg } = label.data

    const newLabel = document.createElementNS(labelNS.svgNS, "g")
    newLabel.setAttribute(
      "transform",
      `translate(${x} ${y}) scale(${labelNS.curScale}) rotate(${deg} ${width * 0.5} ${height * 0.5})`,
    )
    ...
    const labelBody = document.createElementNS(labelNS.svgNS, "rect")
...
export const initializeLabel = (e) => {
  ...
  labelNS.curLabel.setAttribute(
    "transform",
    `translate(${labelNS.startX} ${labelNS.startY}) scale(${labelNS.curScale}) rotate(0 0 0)`,
  )
...
export const createInputBox = (labelBody) => {
  ...
  const inputWrapper = document.createElementNS(labelNS.svgNS, "foreignObject")
...
export const createAnchors = (label) => {
  ...
  const width = parseFloat(label.firstChild.getAttribute("width"))
  const height = parseFloat(label.firstChild.getAttribute("height"))
  const line = document.createElementNS(labelNS.svgNS, "line")
  line.setAttribute("x1", width * 0.5)
  line.setAttribute("y1", 0)
  line.setAttribute("x2", width * 0.5)
  line.setAttribute("y2", -25)
  line.setAttribute("stroke", "#5c6dda")
  line.setAttribute("stroke-width", 3)
  label.appendChild(line)

  const circle = document.createElementNS(labelNS.svgNS, "circle")
  ...
  const infoBox = document.createElementNS(labelNS.svgNS, "rect")
  ...
  const infoTxt = document.createElementNS(labelNS.svgNS, "text")
  infoTxt.setAttribute("y", height + 5)

  const tspan0 = document.createElementNS(labelNS.svgNS, "tspan")
  ...
  const tspan1 = document.createElementNS(labelNS.svgNS, "tspan")
  ...
  const tspan2 = document.createElementNS(labelNS.svgNS, "tspan")
